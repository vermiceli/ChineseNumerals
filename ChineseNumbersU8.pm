...
sub EnglishToChineseNumber {
    my($self) = shift; # grab the first argument as self
    my($enumber) = shift; # grab second argument as input english number
    my($outputtype) = shift; # grab third argument as output type

    if ($outputtype eq "") {
	$outputtype = $default_outputtype; # 'trad'
    }
...
#    print "Output type : $outputtype\n";
    ...
    if ($enumber == 0) {
	return $digits[0]; # BUG doesn't look at output type (pinyin should be ling2)
    }
